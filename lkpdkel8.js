...
// Fungsi untuk menyimpan jawaban ke Supabase
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (elSelect) {
        ...
        textSpan.innerHTML = `<strong>Kelompok 8</strong>`;
        textSpan.style.cssText = "text-align: left; width: 100%; color: #333; padding: 10px 0; font-size: 1rem;";
        const hiddenInput = document.createElement('input');
        hiddenInput.type = 'hidden'; hiddenInput.id = 'kelompok-siswa'; hiddenInput.value = '8';
    ...
    const siswaName = urlParams.get('siswa');
    ...
    if (isReviewMode) {
        ...
        setTimeout(async () => {
            ...
            // ===================================================
                ...
                const rawData = localStorage.getItem('dataReviewSiswa_Kel8');
                ...
                if (rawData) {
                    try {
                        ...
                        if (kelompokData && kelompokData.answer === '8') {
                            dataToLoad = data;
                            console.log('[SISWA MODE] ✅ Data localStorage valid untuk Kelompok 8');
                        } else {
                            console.warn('[SISWA MODE] ⚠️ Data localStorage bukan untuk Kelompok 8');
                        }
                    } catch(e) { 
                        ...
                        try {
                            ...
                            if (error) {
                                ...
                                if (kelompokData && kelompokData.answer === '8') {
                                    ...
                                    console.warn('[SISWA MODE] ⚠️ Data Supabase bukan untuk Kelompok 8');
                                }
                            } else {
    ...
    if (ctx && typeof Chart !== 'undefined') {
        ...
        window.myChart = new Chart(chartCtx, {
            ...
            options: {
                ...
                scales: {
                    x: { type: 'linear', position: 'bottom', title: { display: true, text: 'Hari ke-', font: { weight: 'bold' } }, min: 0, max: 15, ticks: { stepSize: 1 } },
                    y: { title: { display: true, text: 'Harga Emas (ribu Rp/gram)', font: { weight: 'bold' } }, min: 2000, max: 2500, ticks: { stepSize: 50 } }
                },
                plugins: {
...
function isiFormulirOtomatis(data) {
    ...
    const mapSoal = {
        ...
        'Metode Dewi': 'jawab_metode1',
        'Metode Fajar': 'jawab_metode2',
        'Gradien Fajar': 'jawab_gradien',
        'Refleksi': 'jawab_kesimpulan',
        'Alasan Kesimpulan': 'reasonText',
        'Persamaan Final': 'finalEquation',
        'Harga Hari-12': 'jawab5',
        'Hari Target': 'jawab6',
...
function disableInputs() {
    document.querySelectorAll('input, select, textarea, button').forEach(el => {
        if (el.id !== 'btnKembaliClosing' &&
            el.id !== 'commentName' && el.id !== 'commentText' && el.id !== 'btnSaveComment' &&
            !el.closest('#comment-section') && !el.classList.contains('swal2-confirm')) {
            el.disabled = true;
...
// =========================================
// 4. FITUR KOMENTAR ONLINE
// =========================================
function setupOnlineComments() {
    const section = document.createElement('div');
    section.id = 'comment-section';
    section.style.cssText = "width: 90%; max-width: 800px; margin: 30px auto 100px; background: #fff; padding: 25px; border-radius: 15px; box-shadow: 0 10px 25px rgba(0,0,0,0.1); border-top: 5px solid #B8860B;";
    section.innerHTML = `
        <h3 style="margin-top:0; color:#B8860B;"><i class="fa-solid fa-comments"></i> Tanggapan Teman & Guru</h3>
        ...
        <div style="background:#f5f5f5; padding:15px; border-radius:10px;">
            ...
            <button id="btnSaveComment" onclick="sendCommentToSupabase()" style="margin-top:10px; background:#B8860B; color:white; border:none; padding:10px 20px; border-radius:5px; cursor:pointer; font-weight:bold; width:100%;">
    ...
    if (supabaseClient) {
        ...
        if (error) {
            ...
            if (data && data.length > 0) {
                ...
                let myComments = JSON.parse(localStorage.getItem('myComments_Kel8') || '[]');
                myComments.push(commentId);
                localStorage.setItem('myComments_Kel8', JSON.stringify(myComments));
...
async function loadCommentsFromSupabase() {
    ...
    if (data) {
        ...
        const myComments = JSON.parse(localStorage.getItem('myComments_Kel8') || '[]');
        ...
        data.forEach(item => {
            ...
            html += `
                <div style="background:white; margin-bottom:10px; padding:10px; border-radius:8px; border:1px solid #eee;">
                    <div style="font-weight:bold; color:#B8860B; display:flex; justify-content:space-between; align-items:center;">
...
function updateTime() {
    ...
    if (waktuEl) waktuEl.textContent = now.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' });
}
updateTime();
...
function undoPoint() {
    if (window.simpanTitik.length > 0) {
        ...
        if (window.myChart) { window.myChart.data.datasets[0].data.pop(); window.myChart.update(); }
...
function clearAllPoints() {
    window.simpanTitik = [];
    if (window.myChart) { window.myChart.data.datasets[0].data = []; window.myChart.update(); }
    document.getElementById('graphFeedback').innerHTML = '';
}
...
async function checkAnswer(type) {
    ...
    if (type === 'metode1') { 
        ...
        if (VALID_ANSWERS.some(kunci => { const k = kunci.replace("y=",""); return ans === k || ans === "y="+k; })) {
            isCorrect = true;
            message = '<i class="fa-solid fa-check-circle"></i> <strong>Benar!</strong> Rumus akhirnya adalah <strong>y = 50x + 1950</strong>. (atau 50x + 1950).';
        } else {
    ...
    // --- METODE FAJAR ---
        ...
        if (VALID_ANSWERS.some(kunci => { const k = kunci.replace("y=",""); return rawInput === k || rawInput === "y="+k; })) {
            isCorrect = true;
            message = '<i class="fa-solid fa-check-circle"></i> <strong>Sempurna!</strong> Fajar menemukan kenaikan harga emas setiap harinya adalah Rp50.000';
        } else {
    ...
    // --- UJI COBA 1 ---
        ...
        // Kunci: 2550
        if (Math.abs(ans - 2550) <= 0) {
            isCorrect = true;
            message = '<i class="fa-solid fa-check-circle"></i> <strong>Benar!</strong> Pada hari ke-12, harga emas diprediksi <strong>Rp2.550.000/gram</strong>.';
        } else {
            message = '<i class="fa-solid fa-circle-xmark"></i> Salah. Substitusi nilai x = 12 pada persamaan final.';
        }
    }
    // --- UJI COBA 2 ---
    else if (type === 6) { 
        ...
        // Kunci: 21
        if (!isNaN(ans) && Math.abs(ans - 21) <= 0) {
            isCorrect = true;
            message = '<i class="fa-solid fa-check-circle"></i> <strong>Benar!</strong> Harga emas mencapai Rp3.000.000/gram pada hari ke-<strong>21</strong>.';
        } else {
            message = '<i class="fa-solid fa-circle-xmark"></i> Salah. Substitusi y = 3000 pada persamaan final.';
        }
    }
...
async function checkGraphPoints() {
    ...
    // A(2, 2050), B(5, 2200)
    const hasA = points.some(p => Math.abs(p.x - 2) <= 0 && Math.abs(p.y - 2050) <= 0);
    const hasB = points.some(p => Math.abs(p.x - 5) <= 0 && Math.abs(p.y - 2200) <= 0);
    ...
    // KONDISI 2: BARU MENEMUKAN SATU TITIK (A SAJA)
        ...
        feedback.innerHTML = `
            ...
            Tapi <strong>Titik B</strong> (5, 2200) belum ketemu. Cari lagi ya!
        `;
    ...
    // KONDISI 3: BARU MENEMUKAN SATU TITIK (B SAJA)
    else if (!hasA && hasB) {
        feedback.innerHTML = `
            <i class="fa-solid fa-circle-exclamation"></i> <strong>Bagus! Titik B sudah pas.</strong><br>
            Tapi <strong>Titik A</strong> (2, 2050) belum ketemu. Cari lagi ya!
        `;
        feedback.className = 'feedback wrong';
    ...
    // KONDISI 4: BELUM MENEMUKAN SAMA SEKALI
    else {
...
function checkFinalEquation() {
    ...
    if (equation === '50x+1950' || equation === 'y=50x+1950') {
...
function checkDevAnswer(num) {
    ...
    if (num === 1) {
        ...
        if (ans.length > 15) {
            ...
            feedback.innerHTML = '<i class="fa-solid fa-circle-exclamation"></i> Jawaban terlalu singkat.';
            feedback.className = 'feedback wrong';
        }
    }
...
async function confirmSubmission() {
    ...
    const isEq = (v) => { if(!v) return false; const c = v.toLowerCase().replace(/\s/g,'').replace(/,/g, '.'); return c === '50x+1950' || c === 'y=50x+1950'; };
    const pushData = (q, ans, corr) => dataToSave.push({ student_name: sName, question: q, answer: ans, is_correct: corr });

    // === VALIDASI SOAL UTAMA ===
    pushData('Metode Dewi', document.getElementById('jawab_metode1').value, isEq(document.getElementById('jawab_metode1').value));
    pushData('Metode Fajar', document.getElementById('jawab_metode2').value, isEq(document.getElementById('jawab_metode2').value));
    
    let v5 = parseFloat(document.getElementById('jawab5').value.replace(',','.'));
    pushData('Harga Hari-12', document.getElementById('jawab5').value, Math.abs(v5 - 2550) <= 0);
    let v6 = parseFloat(document.getElementById('jawab6').value.replace(',','.'));
    pushData('Hari Target', document.getElementById('jawab6').value, Math.abs(v6 - 21) <= 0);
    ...
    pushData('Koordinat A', `(${coordAx}, ${coordAy})`, coordAx === 2 && coordAy === 2050);
    pushData('Koordinat B', `(${coordBx}, ${coordBy})`, coordBx === 5 && coordBy === 2200);
    
    // 3. Gradien (Metode 2)
    let gVal = parseFloat(document.getElementById('jawab_gradien').value.replace(',','.'));
    pushData('Gradien Fajar', document.getElementById('jawab_gradien').value, Math.abs(gVal - (50)) < 0.1);
    ...
    // 8. Pendapat Kelompok (>15 karakter = benar)
    ...
    // Fase 2: Pilihan Strategi & Identifikasi Variabel
    const pilihanStrategi = document.querySelector('input[name="pilih-strategi"]:checked');
    pushData('Pilihan Strategi', pilihanStrategi ? pilihanStrategi.value : '', true);
    pushData('Alasan Strategi', document.getElementById('diskusi-alasan-strategi').value, true);
    pushData('Identifikasi Variabel', document.getElementById('diskusi-variabel').value, true);
    ...
    const kelompokSiswa = document.getElementById('kelompok-siswa').value || '8';
    localStorage.setItem('kelompok-siswa', kelompokSiswa);
    
    localStorage.setItem('dataReviewSiswa_Kel8', JSON.stringify(dataToSave));
...
function tampilkanStatusJawaban(data) {
    ...
    const soalDiabaikan = ['Info Kelompok', 'nama-siswa', 'Link Presentasi', 'Pilihan Strategi', 'Alasan Strategi', 'Identifikasi Variabel', 'Alasan Kesimpulan', 'KOMENTAR_KEL_1', 'KOMENTAR_KEL_2', 'KOMENTAR_KEL_3', 'KOMENTAR_KEL_4', 'KOMENTAR_KEL_5', 'KOMENTAR_KEL_6', 'KOMENTAR_KEL_7', 'KOMENTAR_KEL_8'];
    ...
    setTimeout(() => {
        document.querySelectorAll('input, select, textarea').forEach(el => {
            if (el.disabled && el.style.backgroundColor &&
                (el.style.backgroundColor === 'rgb(212, 237, 218)' || el.style.backgroundColor === 'rgb(248, 215, 218)')) {
                ...
                icon.innerHTML = isBenar ? '<i class="fa-solid fa-circle-check"></i>' : '<i class="fa-solid fa-circle-xmark"></i>';
                el.insertAdjacentElement('afterend', icon);
            }
        });
        const grafik = data.find(item => item.question === 'Visualisasi Grafik Kartesius');
        if (grafik) {
            ...
            if (grafikFeedback && !grafikFeedback.querySelector('.review-status-icon')) {
                grafikFeedback.innerHTML = grafik.is_correct ? '<i class="fa-solid fa-circle-check"></i> <strong>Grafik Benar!</strong> Titik A dan B terpasang dengan tepat.' : '<i class="fa-solid fa-circle-xmark"></i> <strong>Grafik Belum Tepat.</strong> Titik yang diplot belum sesuai.';
                grafikFeedback.className = grafik.is_correct ? 'feedback correct' : 'feedback wrong';
            }
        }
        const koordinatA = data.find(item => item.question === 'Koordinat A');
        const koordinatB = data.find(item => item.question === 'Koordinat B');
        ...
        if (coordFeedback && (koordinatA || koordinatB)) {
            ...
            coordFeedback.innerHTML = `${aBenar ? '✅' : '❌'} Titik A: <strong>${aBenar ? 'Benar' : 'Salah'}</strong> &nbsp;|&nbsp; ${bBenar ? '✅' : '❌'} Titik B: <strong>${bBenar ? 'Benar' : 'Salah'}</strong>`;
