...
async function checkGraphPoints() {
    ...
    // A(20, 360), B(50, 900)
    const hasA = points.some(p => Math.abs(p.x - 20) <= 5 && Math.abs(p.y - 360) <= 5);
    const hasB = points.some(p => Math.abs(p.x - 50) <= 5 && Math.abs(p.y - 900) <= 5);
...
function checkFinalEquation() {
    ...
    if (equation === '18x' || equation === 'y=18x' || equation === '18x+0' || equation === 'y=18x+0') {
...
async function confirmSubmission() {
    ...
    const isEq = (v) => { if(!v) return false; const c = v.toLowerCase().replace(/\s/g,'').replace(/,/g, '.'); return c === '18x' || c === 'y=18x' || c === '18x+0' || c === 'y=18x+0'; };
    ...
    pushData('Biaya 85 Nasi Kotak', document.getElementById('jawab5').value, Math.abs(v5 - 1530) <= 0);
