...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (elSelect) {
        ...
        textSpan.innerHTML = `<strong>Kelompok 7</strong>`;
        textSpan.style.cssText = "text-align: left; width: 100%; color: #333; padding: 10px 0; font-size: 1rem;";
        const hiddenInput = document.createElement('input');
        hiddenInput.type = 'hidden'; hiddenInput.id = 'kelompok-siswa'; hiddenInput.value = '7';
    ...
    if (isReviewMode) {
        ...
        setTimeout(async () => {
            ...
            // ===================================================
                ...
                const rawData = localStorage.getItem('dataReviewSiswa_Kel7');
                ...
                if (rawData) {
                    try {
                        ...
                        if (kelompokData && kelompokData.answer === '7') {
                            dataToLoad = data;
                            console.log('[SISWA MODE] ✅ Data localStorage valid untuk Kelompok 7');
                        } else {
                            console.warn('[SISWA MODE] ⚠️ Data localStorage bukan untuk Kelompok 7');
                        }
                    } catch(e) { 
                        ...
                        try {
                            ...
                            if (error) {
                                ...
                                if (kelompokData && kelompokData.answer === '7') {
                                    ...
                                    console.warn('[SISWA MODE] ⚠️ Data Supabase bukan untuk Kelompok 7');
                                }
                            } else {
    ...
    if (ctx && typeof Chart !== 'undefined') {
        ...
        window.myChart = new Chart(chartCtx, {
            ...
            options: {
                ...
                scales: {
                    x: { type: 'linear', position: 'bottom', title: { display: true, text: 'Hari ke-', font: { weight: 'bold' } }, min: 0, max: 25, ticks: { stepSize: 1 } },
                    y: { title: { display: true, text: 'Sisa Tinta (%)', font: { weight: 'bold' } }, min: 0, max: 100, ticks: { stepSize: 5 } }
                },
                plugins: {
...
function setupOnlineComments() {
    ...
    section.style.cssText = "width: 90%; max-width: 800px; margin: 30px auto 100px; background: #fff; padding: 25px; border-radius: 15px; box-shadow: 0 10px 25px rgba(0,0,0,0.1); border-top: 5px solid #6A0572;";
    section.innerHTML = `
        <h3 style="margin-top:0; color:#6A0572;"><i class="fa-solid fa-comments"></i> Tanggapan Teman & Guru</h3>
        ...
        <div style="background:#f5f5f5; padding:15px; border-radius:10px;">
            ...
            <button id="btnSaveComment" onclick="sendCommentToSupabase()" style="margin-top:10px; background:#6A0572; color:white; border:none; padding:10px 20px; border-radius:5px; cursor:pointer; font-weight:bold; width:100%;">
    ...
    if (supabaseClient) {
        ...
        if (error) {
            ...
            if (data && data.length > 0) {
                ...
                let myComments = JSON.parse(localStorage.getItem('myComments_Kel7') || '[]');
                myComments.push(commentId);
                localStorage.setItem('myComments_Kel7', JSON.stringify(myComments));
...
async function loadCommentsFromSupabase() {
    ...
    if (data) {
        ...
        const myComments = JSON.parse(localStorage.getItem('myComments_Kel7') || '[]');
        ...
        data.forEach(item => {
            ...
            html += `
                <div style="background:white; margin-bottom:10px; padding:10px; border-radius:8px; border:1px solid #eee;">
                    <div style="font-weight:bold; color:#6A0572; display:flex; justify-content:space-between; align-items:center;">
...
// --- LOGIKA CEK JAWABAN (Tinta Printer: y = -4x + 104) ---
async function checkAnswer(type) {
    ...
    if (type === 'metode1') { 
        ...
        if (VALID_ANSWERS.some(kunci => { const k = kunci.replace("y=",""); return ans === k || ans === "y="+k; })) {
            isCorrect = true;
            message = '<i class="fa-solid fa-check-circle"></i> <strong>Benar!</strong> Rumus akhirnya adalah <strong>y = -4x + 104</strong> (atau -4x + 104).';
        } else {
    ...
    // --- METODE WAHYU ---
        ...
        if (VALID_ANSWERS.some(kunci => { const k = kunci.replace("y=",""); return rawInput === k || rawInput === "y="+k; })) {
            isCorrect = true;
            message = '<i class="fa-solid fa-check-circle"></i> <strong>Sempurna!</strong> Wahyu menemukan pemakaian tinta pulpen sebanyak 4% setiap harinya.';
        } else {
    ...
    // --- UJI COBA 2 ---
        ...
        // Kunci: 26
        if (!isNaN(ans) && Math.abs(ans - 26) <= 0) {
            isCorrect = true;
            message = '<i class="fa-solid fa-check-circle"></i> <strong>Benar!</strong> Tinta akan habis pada hari ke-<strong>26</strong>.';
        } else {
            message = '<i class="fa-solid fa-circle-xmark"></i> Salah. Substitusi nilai y = 0 pada persamaan final.';
        }
    }
...
function checkFinalEquation() {
    ...
    if (equation === '-4x+104' || equation === 'y=-4x+104') {
...
async function confirmSubmission() {
    ...
    const isEq = (v) => { if(!v) return false; const c = v.toLowerCase().replace(/\s/g,'').replace(/,/g, '.'); return c === '-4x+104' || c === 'y=-4x+104'; };
    const pushData = (q, ans, corr) => dataToSave.push({ student_name: sName, question: q, answer: ans, is_correct: corr });

    // === VALIDASI SOAL UTAMA ===
    pushData('Metode Lila', document.getElementById('jawab_metode1').value, isEq(document.getElementById('jawab_metode1').value));
    ...
    pushData('Gradien Wahyu', document.getElementById('jawab_gradien').value, Math.abs(gVal - (-4)) < 0.1);
    ...
    pushData('Info Kelompok', '7', false);
    ...
    const kelompokSiswa = document.getElementById('kelompok-siswa').value || '7';
    localStorage.setItem('kelompok-siswa', kelompokSiswa);
    
    localStorage.setItem('dataReviewSiswa_Kel7', JSON.stringify(dataToSave));
